...
// RouteFrame is the UI window for finding shortest route between cities
public class RouteFrame extends JFrame {
    ...
    public RouteFrame(RoutePlanner rp, String[] cities) {
        ...
        // top header of route planner window
        add(IntelligentTravelPlanner.headerPanel("🗺  Route Planner", CLR_TEAL), BorderLayout.NORTH);

        // text area used to show cities and route results
        out = IntelligentTravelPlanner.styledTA();

        // formatting city list into grid style for display
        ...
        // first row for dropdowns
        JPanel row1 = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 5));
        row1.setBackground(CLR_BG);

        from = new JComboBox<>(cities);
        from.setFont(FONT_BODY);

        to = new JComboBox<>(cities);
        to.setFont(FONT_BODY);

        // default selection difference so route makes sense
        if (cities.length > 1) to.setSelectedIndex(1);

        row1.add(IntelligentTravelPlanner.lbl("Origin:"));
        row1.add(from);
        row1.add(IntelligentTravelPlanner.lbl("Destination:"));
        row1.add(to);
        ...
        // button action for finding route
        findBtn.addActionListener(e -> {
            ...
            // checking invalid same city selection
            ...
            if ("ERROR".equals(result[0])) {
                ...
                // show route result in text area
    ...
    // formats cities into grid layout for display
