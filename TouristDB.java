...
public class TouristDB {
    ...
    // Singleton instance (only one DB object is used in whole program)
    private static TouristDB INSTANCE;

    // city (lower-case) -> list of tourist places in that city
    private final Map<String, List<TouristPlace>> byCity = new HashMap<>();

    // city (lower-case) -> latitude and longitude
    private final Map<String, double[]> coords = new HashMap<>();

    // flags to check if data was loaded successfully
    private boolean spotsLoaded = false;
    private boolean coordsLoaded = false;

    private TouristDB() {
        // load data when object is created
        loadCoords();
        loadSpots();
    }

    // returns single instance of this class (singleton pattern)
    ...
    // returns coordinates of a city if available
    public double[] getCoords(String city) {
        if (city == null) return null;
        return coords.get(city.toLowerCase().trim());
    }

    // returns all known cities from database
    public Set<String> knownCities() {
        return Collections.unmodifiableSet(byCity.keySet());
    }

    /**
     * Returns places in city matching interest.
     * If nothing matches, it returns all places so app doesn't break.
     */
    public List<TouristPlace> placesFor(String city, String interest) {
        ...
        // try flexible matching if exact city not found
        if (all == null) {
        ...
        // if no interest selected, return all places
        if (want.isEmpty()) return new ArrayList<>(all);

        // convert interest into categories
        ...
        // fallback: if nothing matches, return all places
        return matched.isEmpty() ? new ArrayList<>(all) : matched;
    }

    /**
     * Maps user interest to multiple categories so results are richer.
     */
    ...
    // loads city coordinates from CSV file
    private void loadCoords() {
        File f = new File("pakistan_cities.csv");

        // if file not found, API will be used instead
        if (!f.exists()) {
    ...
    // loads tourist places from CSV file
    ...
    /**
     * Backup data so program still works even if CSV files are missing.
     */
    ...
    /**
     * Simple CSV splitter (handles quotes properly).
     */
